...
/*---------- 15. Split Text Animation With GSAP Plugins ----------*/
  ...
  /**************************************
   ***** 18. Floating Contact Box - Follows Scroll *****
   **************************************/
  const floatingContactBox = document.querySelector('.contact-box2-sticky');

  if (floatingContactBox && window.innerWidth > 991) {
    const parentColumn = floatingContactBox.closest('.col-lg-4');
    const contentColumn = parentColumn ? parentColumn.parentElement.querySelector('.col-lg-8') : null;

    if (parentColumn && contentColumn) {
      // Get initial position
      const initialTop = floatingContactBox.getBoundingClientRect().top + window.scrollY;
      const headerOffset = 120; // Space from top of viewport
      let currentY = 0;
      let targetY = 0;
      ...
      function animateFloat() {
        // Ease towards target with smooth boat-like motion
        const ease = 0.08; // Lower = smoother/slower movement
        const diff = targetY - currentY;

        if (Math.abs(diff) > 0.5) {
          currentY += diff * ease;
          floatingContactBox.style.transform = `translateY(${currentY}px)`;

          // Add subtle floating motion when moving
          if (isFloating && Math.abs(diff) > 2) {
            const floatOffset = Math.sin(Date.now() / 300) * 3;
            floatingContactBox.style.transform = `translateY(${currentY + floatOffset}px)`;
          }
        }
      ...
      // Calculate target position on scroll
      function updatePosition() {
        const scrollTop = window.scrollY;
        const parentRect = parentColumn.getBoundingClientRect();
        const contentRect = contentColumn.getBoundingClientRect();
        const boxHeight = floatingContactBox.offsetHeight;

        // Calculate the bottom boundary (don't go past content)
        const contentBottom = contentRect.bottom + scrollTop;
        const maxScroll = contentBottom - initialTop - boxHeight - 40;

        // Calculate how much we should move
        const scrollDistance = scrollTop - (initialTop - headerOffset);

        if (scrollDistance > 0) {
          // We've scrolled past the initial position
          isFloating = true;
          floatingContactBox.classList.add('is-stuck');
          targetY = Math.min(scrollDistance, maxScroll);
        } else {
          // We're above the initial position
          isFloating = false;
          floatingContactBox.classList.remove('is-stuck');
          targetY = 0;
        }
      }
